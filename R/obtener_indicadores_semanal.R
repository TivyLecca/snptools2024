obtener_indicadores_semanal <- function(descargas = descargas, calas = calas){
  # library(dplyr)
  # require(lubridate)
  #
  ...
  # }
  #
  # descargas$temporada_semana = paste0(descargas$temporada, "-", descargas$semana)
  #x = descargas[descargas$temporada_semana %in% descargas$temporada_semana[100], ]

  indicadores = lapply(split(descargas, descargas$temporada, drop = TRUE),
                       function(y) {
                         #y =   descargas[descargas$temporada %in% descargas$temporada[1],]
                         #x = y[y$semana %in% y$semana[1], ]

                         por_semana = lapply(split(y, y$semana, drop = TRUE),
                                             function(x) {
                                               ...
                                               if(dim(w_calas)[1]>0){
                                                 ...
                                                 area        = suppressWarnings(vmsR:::covered_area2(dc = w_calas$dc, lat = w_calas$latitud, grado = 1/12))
                                               ...
                                               cbind.data.frame(semana, temporada, barcos_registrados, viajes_registrados, barcos, viajes, duracion, duracion_median, lances,
                         ...
                         por_semana <- por_semana %>% lapply(as.data.frame) %>% bind_rows()
